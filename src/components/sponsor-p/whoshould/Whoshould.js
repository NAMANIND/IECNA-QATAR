...
function Whoshould() {
  ...
  return (
    <div className="flex flex-col pt-28 pb-28 bg-white">
      <motion.div
        initial={offscreen}
        whileInView={onscreen}
        viewport={{ once: true, amount: 0.3 }}
        className={`self-center text-5xl text-center text-black leading-[61.92px] max-md:max-w-full max-md:text-4xl ${anton.className}`}
      >
        WHO SHOULD SPONSOR?
      </motion.div>
            ...
            {cardData.map((card, index) => (
              ...
              >
                <div className="flex flex-col max-md:mt-10 group">
                  ...
                  ${index % 3 === 1 ? "bg-[#51B6FF]" : "bg-black"}
                  `}
                  />
                </div>
                <div className="text-xl font-medium tracking-tighter leading-7 text-black flex flex-col justify-between max-md:mt-10">
